...
fn test_round_trip_recovery_spherical_earth_polar() {
    ...
    let coord_a = build_coord(EARTH_GM, 2.93e7, 3650.0, [0.0, 0.0, 2.93e7], [0.0, 3650.0, 0.0], &body);
    let coord_b = build_coord(EARTH_GM, 2.95e7, 3640.0, [0.0, 0.0, 2.95e7], [0.0, 3640.0, 0.0], &body);
...
fn test_round_trip_recovery_spherical_mars_like() {
    ...
    let coord_a = build_coord(target_gm, 6.0e6, 3500.0, [6.0e6, 0.0, 0.0], [0.0, 3500.0, 0.0], &body);
    let coord_b = build_coord(target_gm, 7.0e6, 3300.0, [7.0e6, 0.0, 0.0], [0.0, 3300.0, 0.0], &body);
...
fn test_round_trip_recovery_with_j2_mid_latitude() {
    ...
    let coord_a = build_coord(EARTH_GM, r, 3650.0, [r * 0.5, 0.0, z], [0.0, 3650.0, 0.0], &body);
    let r_b = 2.95e7;
    let z_b = r_b * (1.0_f64 / 3.0_f64).sqrt();
    let coord_b = build_coord(EARTH_GM, r_b, 3640.0, [r_b * 0.5, 0.0, z_b], [0.0, 3640.0, 0.0], &body);
...
fn test_j2_sensitivity_changes_result() {
    ...
    let coord_a = build_coord(EARTH_GM, 2.93e7, 3650.0, [2.93e7, 0.0, 0.0], [0.0, 3650.0, 0.0], &body_with_j2);
    let coord_b = build_coord(EARTH_GM, 2.95e7, 3640.0, [2.95e7, 0.0, 0.0], [0.0, 3640.0, 0.0], &body_with_j2);
...
fn test_error_insufficient_radial_separation() {
    ...
    let coord_a = build_coord(EARTH_GM, 2.93e7, 3650.0, [2.93e7, 0.0, 0.0], [0.0, 3650.0, 0.0], &body);
    let coord_b = build_coord(EARTH_GM, 2.93e7, 3650.0, [2.93e7, 0.0, 0.0], [0.0, 3650.0, 0.0], &body);
...
fn test_higher_altitude_clock_drifts_higher() {
    ...
    let body = CentralBody::<f64>::new(EARTH_GM, EARTH_RADIUS_EQUATORIAL, 0.0);
    let coord_a = build_coord(EARTH_GM, 2.0e7, 4000.0, [2.0e7, 0.0, 0.0], [0.0, 4000.0, 0.0], &body);
    let coord_b = build_coord(EARTH_GM, 4.0e7, 3000.0, [4.0e7, 0.0, 0.0], [0.0, 3000.0, 0.0], &body);
